	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				/* translators: %s: plugin name. */
				'updating'                        => __( 'Updating Plugin: %s', 'pinnacle' ),
				...
				'notice_cannot_install_activate'  => __( 'There are one or more required or recommended plugins to install, update or activate.', 'pinnacle' ),
		...
		public function notices() {
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				add_settings_error( 'tgmpa', 'pinnacle', $rendered, $this->get_admin_notice_class() );
			}
		...
		protected function create_user_action_links_for_notice( $install_count, $update_count, $activate_count, $line_template ) {
			...
			if ( current_user_can( 'install_plugins' ) ) {
				if ( $install_count > 0 ) {
					$action_links['install'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['install_link'], $install_count, 'pinnacle' ),
				...
				if ( $update_count > 0 ) {
					$action_links['update'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['update_link'], $update_count, 'pinnacle' ),
						esc_url( $this->get_tgmpa_status_url( 'update' ) )
					);
				}
			}
			...
			if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
				$action_links['activate'] = sprintf(
					$link_template,
					translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'pinnacle' ),
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_source_type_text( $type, $source ) {
			...
			switch ( $type ) {
				case 'repo':
					...
					break;
				case 'external':
					$string = '<a href="'.$source.'" target="_blank">'.__( 'External Source', 'pinnacle' ).'</a>';
					break;
				case 'bundled':
		...
		protected function get_plugin_status_text( $slug ) {
			...
			return sprintf(
				...
				_x( '%1$s, %2$s', 'Install/Update Status', 'pinnacle' ),
				$install_status,
				$update_status
			);
		}
		...
		public function no_items() {
			echo esc_html__( 'No plugins to install, update or activate.', 'pinnacle' ) . ' <a href="' . esc_url( self_admin_url() ) . '"> ' . esc_html__( 'Return to the Dashboard', 'pinnacle' ) . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$actions['install'] = __( 'Install %2$s', 'pinnacle' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['update'] = __( 'Update %2$s', 'pinnacle' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['activate'] = __( 'Activate %2$s', 'pinnacle' );
				}
			}
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins were selected to be installed. No action taken.', 'pinnacle' );
					} else {
						$message = __( 'No plugins were selected to be updated. No action taken.', 'pinnacle' );
					}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins were selected to be activated. No action taken.', 'pinnacle' ), '</p></div>';
