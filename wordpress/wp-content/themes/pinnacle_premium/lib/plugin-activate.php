...
function kadence_register_required_plugins() {
	...
	if($license == 'pps' || $license == 'ktm'  ) {
		// do nothing
	} else if(isset($pinnacle['kt_revslider_notice']) && $pinnacle['kt_revslider_notice'] == 1 ) {
			$addons[] = array(
				'name'     				=> 'Revolution Slider',
				'slug'     				=> 'revslider',
				'source'   				=> 'https://s3.amazonaws.com/ktupdates/api/876709873876287/revslider.zip',
				'required' 				=> false,
				'version' 				=> '5.4.5',
	...
	if(isset($pinnacle['kt_pagebuilder_notice']) && $pinnacle['kt_pagebuilder_notice'] == 1) {
			$suggested[] = array(
			'name'     				=> 'Page Builder by SiteOrigin', // The plugin name
			'slug'     				=> 'siteorigin-panels', // The plugin slug (typically the folder name)
			);
	}
	if(isset($pinnacle['kt_tinymce_notice']) && $pinnacle['kt_tinymce_notice'] == 1) {
			$suggested[] = array(
	...
	$config = array(
		...
		'strings'      		=> array(
			...
			'menu_title'                       			=> __( 'Theme Plugins', 'pinnacle' ),
