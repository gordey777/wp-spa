...
<?php if(!empty($instance['soundcloud'])):?><a href="<?php echo esc_url($instance['soundcloud']); ?>" class="soundcloud_link" title="Soundcloud" target="_blank" data-toggle="tooltip" data-placement="top" data-original-title="Soundcloud"><i class="kt-icon-soundcloud"></i></a><?php endif;?>
<?php if(!empty($instance['rss'])):?><a href="<?php echo esc_url($instance['rss']); ?>" class="rss_link" title="RSS" target="_blank" data-toggle="tooltip" data-placement="top" data-original-title="RSS"><i class="kt-icon-feed"></i></a><?php endif;?>
  ...
  <p>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('soundcloud')); ?>"><?php _e('Soundcloud:', 'pinnacle'); ?></label>
    ...
    <li class="clearfix postclass kt-top-featured">
        <a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>" class="recentpost_featimg">
          <?php 
                ...
                <img src="<?php echo esc_url($image); ?>" alt="<?php the_title_attribute(); ?>" <?php echo 'width="'.esc_attr($image_width).'" height="'.esc_attr($image_height).'"';?> class="iconhover" style="display:block;">
                </a>
        ...
        <span class="recentpost_date color_gray"><?php echo get_the_date(get_option( 'date_format' )); ?></span>
          <?php  if($feature == "true" && $i == 1) { ?>
           ...
           <?php if(function_exists('the_advanced_excerpt') ) {
                                  ...
                                  <p><?php echo pinnacle_excerpt(20); ?> <a href="<?php the_permalink() ?>"><?php global $pinnacle; if(!empty($pinnacle['post_readmore_text'])) {$readmore = $pinnacle['post_readmore_text'];} else {$readmore = __('Read More', 'pinnacle');} echo $readmore; ?></a></p>
                                  <?php } ?> 
           </span>
           <?php } ?>
      </li>
...
class kad_testimonial_slider_widget extends WP_Widget {
  ...
  public function widget($args, $instance) {
    ...
    if(!empty($instance['thecat'])) {$group = $instance['thecat'];} else {$group = '';}
...
?>
    ...
    <div id="testimonial-c-case-<?php echo esc_attr($carousel_rn);?>">
    <?php if ( $title ) echo $before_title . $title . $after_title; ?>
        <div class="fredcarousel">
          <div id="carouselcontainer-<?php echo esc_attr($carousel_rn);?>" class="rowtight kt-testimonail-carousel">
          <div id="testimonial-carousel-<?php echo esc_attr($carousel_rn);?>" class="kad-testimonial-carousel initcaroufedsel-intrinsic" data-carousel-container="#carouselcontainer-<?php echo $carousel_rn;?>" data-carousel-transition="700" data-carousel-scroll="<?php echo esc_attr($scroll); ?>" data-carousel-auto="true" data-carousel-speed="<?php echo esc_attr($speed); ?>" data-carousel-id="<?php echo esc_attr($carousel_rn);?>" data-carousel-sxl="<?php echo esc_attr($md);?>" data-carousel-xl="<?php echo esc_attr($md);?>" data-carousel-md="<?php echo esc_attr($md);?>" data-carousel-sm="<?php echo esc_attr($sm);?>" data-carousel-xs="<?php echo esc_attr($xs);?>" data-carousel-ss="<?php echo esc_attr($ss);?>">
            <?php  while ($wp_query->have_posts()) : $wp_query->the_post(); ?>
            <div class="<?php echo $itemsize;?> t_item">
              <div class="grid_item testimonial_item all">
                <div class="testimonialbox clearfix" style="<?php echo esc_attr($bg_color);?> <?php echo esc_attr($test_color);?>">
                <?php global $post; ?>
      ...
      <?php endwhile; ?>
        ...
        <a class="prev kt-icon-arrow-left test-prev" id="prevport-<?php echo $carousel_rn; ?>" style="<?php echo esc_attr($bg_color);?> <?php echo esc_attr($test_color);?>" href="#"></a>
        <a class="next kt-icon-arrow-right test-next" id="nextport-<?php echo $carousel_rn; ?>" style="<?php echo esc_attr($bg_color);?> <?php echo esc_attr($test_color);?>" href="#"></a>
    </div>
