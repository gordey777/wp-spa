...
class RW_Taxonomy_Meta_Kad {
	...
	function check_field_upload() {
		...
		if ( $this->has_field( 'file' ) ) {
			...
			\$('body').on('click', '.rwtm-file-upload', function(){
				...
				template += ' (<a class=\"rwtm-delete-file\" href=\"#\">" . __( 'Delete', 'pinnacle' ) . "</a>)';
		...
		\$('body').on('click', '.rwtm-image-upload', function(){
			...
			var frame = wp.media({
				multiple : true,
				title    : \"" . __( 'Select Image', 'pinnacle' ) . "\",
				library  : {
					type: 'image'
				}
			});
	...
	function show_field_file( $field, $meta ) {
		...
		echo "<a href='#' class='rwtm-file-upload button' data-field='{$field['id']}'>" . __( 'Select File', 'pinnacle' ) . "</a>";
		echo '</td>';
	}
	...
	function show_field_image( $field, $meta ) {
		...
		echo "<a href='#' class='rwtm-image-upload button' data-field='{$field['id']}'>" . __( 'Select Image', 'pinnacle' ) . "</a>";
		echo '</td>';
	}
