...
function kadence_custom_excerpt_more( $excerpt ) {
  ...
  if( has_excerpt() ) {
      global $pinnacle; 
      if(isset($pinnacle['custom_excerpt_readmore']) && $pinnacle['custom_excerpt_readmore'] == 1) {
        if(!empty($pinnacle['post_readmore_text'])) {$readmore = $pinnacle['post_readmore_text'];} else { $readmore =  __('Read More', 'ascend') ;}
        $excerpt_more = '&hellip; <a class="kt-excerpt-readmore" aria-label="' . get_the_title() . '"" href="' . get_permalink() . '">'. $readmore . '</a>';
      }
  }
