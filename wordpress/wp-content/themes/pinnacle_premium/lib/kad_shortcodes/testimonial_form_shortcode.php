	...
	if(isset($_POST['submitted']) && wp_verify_nonce( $_POST['post-title-nonce'], 'post-title' ) ) {
		...
		if (!isset($kt_feed_error)){
			...
			$post_id = wp_insert_post( array(
				...
				// For images not using at the moment.
		...
		$post_content = wp_filter_kses($post_content);
		$post_title = wp_filter_kses($post_title);

		$sitename = get_bloginfo('name');
		$subject = '['.esc_attr($sitename) . __(" Testimonial Post", "pinnacle").'] '. __("From ", "pinnacle"). esc_attr($post_title);
