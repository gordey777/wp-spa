...
function kad_portfolio_shortcode_function( $atts, $content) {
	...
	if($orderby == 'menu_order' || $orderby == 'title') {$order = 'ASC';} else {$order = 'DESC';} 
			if(empty($cat)) { 
				$portfolio_cat_slug = ''; 
				$portfolio_cat_ID = ''; 
			} else {
			...
			if ($masonry == 'true') {$slideheight = '';}

			if (($fullwidth == "true") && ($usefullwidthdiv == "true")) { echo '<div class="p-shortcode-container kt-panel-row-full-stretch panel-row-style-wide-content">'; } 
	  		if ($filter == 'true') { 
             ...
             if ($filter == 'true') { $shortcodeclasses[] = 'kad_pfilter_class';}
		...
		ob_start(); ?>
				<div id="portfoliowrapper-<?php echo esc_attr($id);?>" class="rowtight reinit-isotope init-isotope-intrinsic <?php echo implode(" ", $shortcodeclasses);?>" style="margin-bottom:0;" data-fade-in="<?php echo esc_attr($animate);?>" data-iso-selector=".p-item" data-iso-style="<?php echo esc_attr($isostyle);?>" data-iso-filter="true"> 
            	<?php   $wp_query = null; 
