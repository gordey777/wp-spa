...
if ( get_option( 'kt_api_manager_pinnacle_premium_activated' ) == 'Activated' ) {
	...
	add_action( 'admin_notices', 'pinnacle_update_inactive_notice');
}
function pinnacle_update_inactive_notice() {
	...
	if(isset($screen->id)) {
		if('update-core' == $screen->id || 'themes' == $screen->id) {
			echo '<div id="message" class="error" style="background: #dc3232; color: white;">';
		        echo '<p>';
		        printf( __( 'This theme is not activated! To get theme updates you must activate your theme! %sClick here%s to activate the license api key.', 'pinnacle' ), '<a style="color: white;" href="' . esc_url( admin_url( 'themes.php?page=kt_api_manager_dashboard' ) ) . '">', '</a>' ); 
		        echo '</p>';
		    echo '</div>';
		}
	}
