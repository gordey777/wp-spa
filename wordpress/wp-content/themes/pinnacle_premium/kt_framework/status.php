...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    class Kadence_Status {
        ...
        public function kadence_status_page() {
            add_management_page(__( 'Kadence System Status', 'pinnacle' ), __( 'System Status', 'pinnacle' ), 'edit_theme_options', 'kadence-status', array($this, 'status_page_content') );
        }
        ...
        <tr>
            <td data-export-label="WP Content URL">
                <?php esc_html_e( 'WP Content URL', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="WP Version">
                <?php esc_html_e( 'WP Version', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="WP Multisite">
                <?php esc_html_e( 'WP Multisite', 'pinnacle' ); ?>:
            </td>
        ...
        <tr>
            <td data-export-label="Permalink Structure">
                <?php esc_html_e( 'Permalink Structure', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="Front Page Display">
                <?php esc_html_e( 'Front Page Display', 'pinnacle' ); ?>:
            </td>
...
?>
                <tr>
                    <td data-export-label="Front Page">
                        <?php esc_html_e( 'Front Page', 'pinnacle' ); ?>:
                    </td>
                    <td>
                ...
                <tr>
                    <td data-export-label="Posts Page">
                        <?php esc_html_e( 'Posts Page', 'pinnacle' ); ?>:
                    </td>
                    <td>
...
?>
        <tr>
            <td data-export-label="WP Memory Limit">
                <?php esc_html_e( 'WP Memory Limit', 'pinnacle' ); ?>:
            </td>
            <td>
<?php
                    ...
                    if ( $memory < 40000000 ) {
                        echo '<mark class="error">' . sprintf( __( '%s - We recommend setting memory to at least 40MB. See: <a href="%s" target="_blank">Increasing memory allocated to PHP</a>', 'pinnacle' ), esc_html($sysinfo['wp_mem_limit']['size']), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</mark>';
                    } else {
...
?>
        ...
        <tr>
            <td data-export-label="Database Table Prefix">
                <?php esc_html_e( 'Database Table Prefix', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="WP Debug Mode">
                <?php esc_html_e( 'WP Debug Mode', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="Language">
                <?php esc_html_e( 'Language', 'pinnacle' ); ?>:
            </td>
            <td>
    ...
    <table class="kt_status_table widefat" cellspacing="0" id="status">
        ...
        <tr>
            <th colspan="3" data-export-label="Server Environment">
                <?php esc_html_e( 'Server Environment', 'pinnacle' ); ?>
            </th>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td data-export-label="Server Info">
                <?php esc_html_e( 'Server Info', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="PHP Version">
                <?php esc_html_e( 'PHP Version', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="ABSPATH">
                <?php esc_html_e( 'ABSPATH', 'pinnacle' ); ?>:
            </td>
            <td>
        ...
        <?php if ( function_exists( 'ini_get' ) ) { ?>
            <tr>
                <td data-export-label="PHP Memory Limit"><?php esc_html_e( 'PHP Memory Limit', 'pinnacle' ); ?>:</td>
                <td><?php echo esc_html($sysinfo['php_mem_limit']); ?></td>
            </tr>
            <tr>
                <td data-export-label="PHP Post Max Size"><?php esc_html_e( 'PHP Post Max Size', 'pinnacle' ); ?>:</td>
                <td><?php echo esc_html($sysinfo['php_post_max_size']); ?></td>
            </tr>
            <tr>
                <td data-export-label="PHP Time Limit"><?php esc_html_e( 'PHP Time Limit', 'pinnacle' ); ?>:</td>
                <td><?php echo esc_html($sysinfo['php_time_limit']); ?></td>
            </tr>
            <tr>
                <td data-export-label="PHP Max Input Vars"><?php esc_html_e( 'PHP Max Input Vars', 'pinnacle' ); ?>:</td>
                <td><?php echo esc_html($sysinfo['php_max_input_var']); ?></td>
            </tr>
            <tr>
                <td data-export-label="PHP Display Errors"><?php esc_html_e( 'PHP Display Errors', 'pinnacle' ); ?>:</td>
                <td><?php
        ...
        <?php } ?>
        <tr>
            <td data-export-label="SUHOSIN Installed"><?php esc_html_e( 'SUHOSIN Installed', 'pinnacle' ); ?>:</td>
            <td>
        ...
        <tr>
            <td data-export-label="MySQL Version"><?php esc_html_e( 'MySQL Version', 'pinnacle' ); ?>:</td>
            <td><?php echo esc_html($sysinfo['mysql_ver']); ?></td>
        </tr>
        <tr>
            <td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'pinnacle' ); ?>:</td>
            <td><?php echo esc_html($sysinfo['max_upload_size']); ?></td>
        </tr>
        <tr>
            <td data-export-label="Default Timezone is UTC">
                <?php esc_html_e( 'Default Timezone is UTC', 'pinnacle' ); ?>:
            </td>
            <td>
<?php
                if ( $sysinfo['def_tz_is_utc'] === 'false' ) {
                    echo '<mark class="error">' . '&#10005; ' . sprintf( __( 'Default timezone is %s - it should be UTC', 'pinnacle' ), esc_html(date_default_timezone_get()) ) . '</mark>';
                } else {
                    echo '<mark class="yes">' . '&#10004;' . '</mark>';
                } 
?>
        ...
        <?php
            ...
            if ( $sysinfo['fsockopen_curl'] === 'true' ) {
                ...
                $posting['fsockopen_curl']['note']    = esc_html__( 'Your server does not have fsockopen or cURL enabled - cURL is used to communicate with other servers. Please contact your hosting provider.', 'pinnacle' ) . '</mark>';
            }
    ...
    <table class="kt_status_table widefat" cellspacing="0" id="status">
        ...
        <tr>
            <th colspan="3" data-export-label="Active Plugins (<?php echo esc_html(count( (array) get_option( 'active_plugins' ) ) ); ?>)">
                <?php esc_html_e( 'Active Plugins', 'pinnacle' ); ?> (<?php echo esc_html(count( (array) get_option( 'active_plugins' ) ) ); ?>)
            </th>
        </tr>
        </thead>
        <tbody>
            ...
            foreach ( $sysinfo['plugins'] as $name => $plugin_data ) {
                ...
                if ( ! empty( $plugin_data['Name'] ) ) {
                    ...
                    if ( ! empty( $plugin_data['PluginURI'] ) ) {
                        $plugin_name = '<a href="' . esc_url( $plugin_data['PluginURI'] ) . '" title="' . esc_attr__( 'Visit plugin homepage', 'pinnacle' ) . '">' . esc_html($plugin_name) . '</a>';
                    }
?>
                    <tr>
                        <td><?php echo $plugin_name; ?></td>
                        <td>
                            <?php echo sprintf( _x( 'by %s', 'by author', 'pinnacle' ), $plugin_data['Author'] ) . ' &ndash; ' . esc_html( $plugin_data['Version'] ) . $version_string . $network_string; ?>
                        </td>
                    </tr>
<?php
                }
    ...
    <table class="kt_status_table widefat" cellspacing="0" id="status">
        <thead>
        <tr>
            <th colspan="3" data-export-label="Theme"><?php esc_html_e( 'Theme', 'pinnacle' ); ?></th>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td data-export-label="Name"><?php esc_html_e( 'Name', 'pinnacle' ); ?>:</td>
            <td><?php echo esc_html($sysinfo['theme']['name']); ?></td>
        </tr>
        <tr>
            <td data-export-label="Version"><?php esc_html_e( 'Version', 'pinnacle' ); ?>:</td>
            <td>
<?php
                echo esc_html($sysinfo['theme']['version']);

                if ( ! empty( $theme_version_data['version'] ) && version_compare( $theme_version_data['version'], $active_theme->Version, '!=' ) ) {
                    echo ' &ndash; <strong style="color:red;">' . esc_html($theme_version_data['version']) . ' ' . esc_html__( 'is available', 'pinnacle' ) . '</strong>';
                }
?>
            </td>
        </tr>
        <tr>
            <td data-export-label="Author URL"><?php esc_html_e( 'Author URL', 'pinnacle' ); ?>:</td>
            <td><?php echo esc_url($sysinfo['theme']['author_uri']); ?></td>
        </tr>
        <tr>
            <td data-export-label="Child Theme"><?php esc_html_e( 'Child Theme', 'pinnacle' ); ?>:</td>
            <td>
            <?php
