	...
	if(isset($pinnacle['outofstocktag']) && $pinnacle['outofstocktag'] == 1) {
		...
		$availability = $product->get_availability();
	 	...
	 	if (! $product->is_in_stock() ) : 
 			if(!empty($pinnacle['sold_placeholder_text'])) {
 				$sold_text = $pinnacle['sold_placeholder_text'];
 			} else {
 				$sold_text = __( 'Sold', 'pinnacle');
 			} 
    		echo '<span class="onsale headerfont kad-out-of-stock">' . $sold_text . '</span>'; 

      	elseif ($product->is_on_sale()) : 
        	if(!empty($pinnacle['sale_placeholder_text'])) {
        		$sale_text = $pinnacle['sale_placeholder_text'];
        	} else {
        		$sale_text = __( 'Sale!', 'pinnacle');
        	} 
